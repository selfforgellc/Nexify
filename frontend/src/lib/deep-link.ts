/**
 * deep-link.ts — Handler for nexify:// deep links in the Tauri desktop app.
 *
 ...
 *   nexify://research/{session_id}   → open a research session
 *   nexify://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `nexify://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `nexify://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `nexify:` scheme.
 *
 * @example
 * parseDeepLink("nexify://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("nexify://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "nexify:") return null;

    // The URL constructor treats "nexify://research/abc123" such that
