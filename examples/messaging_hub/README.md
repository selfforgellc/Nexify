...
nexify orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting nexify to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- nexify installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the nexify channel docs).
...
nexify supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/nexify/config.toml` for the full list of channel and agent
options.
...
schedule. See `src/nexify/recipes/data/operators/` for examples.
