...
TOPICS = [
    # Narrative — opinionated takes that happen to be about nexify
...
FACTS = [
    ...
    "In nearly all personal AI projects today, the local component is a thin orchestration layer, while the brain lives in someone else data center. Your most personal data routes through cloud APIs, with their latency, their cost, and their terms of service. We built nexify to fix this.",
    "nexify is structured around five composable primitives: Intelligence, Engine, Agents, Tools and Memory, and Learning. Each primitive can be benchmarked, substituted, and optimized independently.",
    "nexify includes hardware-agnostic telemetry that profiles energy consumption across NVIDIA GPUs, AMD GPUs, and Apple Silicon. Energy and dollar cost are first-class design constraints alongside accuracy.",
    ...
    "nexify is open source under Apache 2.0, built at Stanford at Hazy Research and the Scaling Intelligence Lab at SAIL. Because the tools for studying and building local-first AI should be available to everyone.",
    "nexify supports 27 channel integrations including Slack, Discord, Telegram, WhatsApp. Adding a new channel is one file implementing BaseChannel with connect, send, and disconnect.",
    "nexify supports multiple inference engines: Ollama, vLLM, SGLang, llama.cpp. jarvis init picks the right one for your hardware.",
    "Install nexify by running git clone https://github.com/open-jarvis/nexify.git then cd nexify then uv sync. Use jarvis init to auto-detect hardware and configure the engine.",
    "nexify memory and RAG supports four backends: SQLite FTS5 for keyword search, FAISS for vector similarity, ColBERT for token-level matching, and BM25 for probabilistic retrieval.",
    "nexify ships with nine example projects: deep_research, code_companion, messaging_hub, scheduled_ops, browser_assistant, security_scanner, daily_digest, doc_qa, and multi_model_router.",
]
...
def main() -> None:
    ...
    from nexify.core.events import EventType
    ...
    # Clean and index
    db_path = os.path.expanduser("~/.nexify/memory.db")
    ...
    parent_ts = slack_send(
        "*nexify Twitter Bot — Overnight Preview*\n"
    ...
    while True:
        ...
        tool_log.clear()
        r = j.ask_full(
            "You are @nexifyAI — a researcher/dev who believes local-first "
            ...
            'server. we built nexify to fix that"\n'
            ...
            "Link: https://github.com/open-jarvis/nexify",
            agent="orchestrator",
