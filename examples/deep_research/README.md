...
using nexify. The assistant uses an orchestrator agent loop with web
...
- nexify installed (`uv sync --extra dev` from the repo root)
...
- **Use the recipe programmatically** -- load the TOML with
  `nexify.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/nexify/recipes/) -- composable TOML configs that
