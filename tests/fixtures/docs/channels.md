...
nexify ships adapters for many messaging platforms.
...
Create a subclass of `BaseChannel` in `src/nexify/channels/your_channel.py`. Decorate with `@ChannelRegistry.register("name")`. Implement `connect`, `disconnect`, `send`, `status`, `list_channels`, and `on_message`.
