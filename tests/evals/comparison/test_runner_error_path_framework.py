...
default of ``"nexify"`` to silently mislabel data from foreign
...
class TestErrorPathFrameworkPropagation:
    def test_failed_task_keeps_backend_framework_name(self, tmp_path: Any) -> None:
        ...
        backend's framework_name, not the dataclass default 'nexify'."""
        cfg = RunConfig(
            benchmark="mock",
            backend="hermes",
            model="mock-model",
        ...
        # NOT from the EvalResult dataclass default of "nexify".
    ...
    def test_default_framework_when_backend_missing_attr(self, tmp_path: Any) -> None:
        ...
        a sensible default (the ABC's "nexify"), via getattr fallback."""
        ...
        # the conservative "nexify" tag rather than crashing.
        assert results[0].framework == "nexify"
