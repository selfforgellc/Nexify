"""Tests for nexify.evals.backends.external.hermes_agent.HermesBackend."""
...
from nexify.evals.backends.external._subprocess_runner import SubprocessResult
    ...
    return ThirdPartyConfig(
        entries={
            "hermes": ThirdPartyEntry(
                ...
                runner_script=(
                    "src/nexify/evals/backends/external/_runners/hermes_runner.py"
                ),
                python_executable="",
            )
        }
...
class TestHermesBackend:
    def test_generate_full_builds_correct_subprocess_command(
        ...
        with (
            ...
            patch(
                "nexify.evals.backends.external.hermes_agent.verify_commit_pin",
            ),
            patch(
                "nexify.evals.backends.external.hermes_agent.run_one_shot",
            ) as mock_run,
        ):
