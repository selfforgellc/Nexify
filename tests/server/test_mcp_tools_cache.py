...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("nexify.mcp.transport.StreamableHTTPTransport"),
        patch("nexify.mcp.client.MCPClient"),
        patch("nexify.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
