"""Verify that importing nexify.tools registers all built-in tools."""
...
def _reload_tool_modules() -> None:
    """Reload all nexify.tools.* submodules to re-trigger @register decorators.

    The autouse ``_clean_registries`` fixture clears all registries before each
    test.  A plain ``import nexify.tools`` won't re-register because the
