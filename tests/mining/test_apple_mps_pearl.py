"""Tests for nexify.mining.apple_mps_pearl."""
...
_AVAIL = "nexify.mining._install.pearl_packages_available"
...
def test_detect_requires_macos(hopper_hw):
    from nexify.mining.apple_mps_pearl import AppleMpsPearlProvider
...
def test_detect_supported_when_torch_mps_available(apple_hw):
    ...
    with (
        patch("nexify.mining._install.pearl_packages_available", return_value=True),
        patch(
            "nexify.mining.apple_mps_pearl._torch_mps_available",
...
def test_start_uses_mps_miner_module(mps_config, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nexify.mining.cpu_pearl._sidecar_path", lambda: sidecar)
    monkeypatch.setattr("nexify.mining.cpu_pearl._log_dir", lambda: log_dir)
    monkeypatch.setattr("nexify.mining.apple_mps_pearl._log_dir", lambda: log_dir)
    ...
    target = "nexify.mining.apple_mps_pearl.PearlSubprocessLauncher"
    ...
    assert (
        ...
        == "nexify.mining._mps_miner_loop_main"
    )
...
def test_mps_noisy_gemm_plain_proof_verifies_when_mps_available():
    ...
    from nexify.mining._mps_miner_loop_main import (
        MpsNoisyGemmAdapter,
