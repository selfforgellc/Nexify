...
from nexify.cli.init_cmd import _next_steps_text
...
class TestInitEmptyModelFallback:
    def test_init_no_model_shows_warning(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("nexify.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("nexify.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("nexify.cli.init_cmd.recommend_model", return_value=""),
            mock.patch("nexify.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitDownloadDispatch:
    def test_init_ollama_download_calls_ollama_pull(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            mock.patch("nexify.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch(
                "nexify.cli.init_cmd.ollama_pull",
                return_value=True,
            ) as mock_pull,
            mock.patch("nexify.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitPrivacyHook:
    def test_init_shows_privacy_summary(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("nexify.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("nexify.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("nexify.cli.init_cmd.PrivacyScanner") as MockScanner,
        ):
            from nexify.cli.scan_cmd import ScanResult
