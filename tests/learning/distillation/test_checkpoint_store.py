"""Tests for nexify.learning.distillation.checkpoint.store module."""
...
def _setup_isolated_repo_root(tmp_path: Path) -> Path:
    """Create a fake nexify-home directory tree with config files for the
    CheckpointStore to track. Returns the root."""
    root = tmp_path / "nexify_home"
...
class TestCheckpointStoreInit:
    ...
    def test_creates_repo_with_baseline_commit(self, tmp_path: Path) -> None:
        from nexify.learning.distillation.checkpoint.store import (
            CheckpointStore,
        )
    ...
    def test_init_refuses_inside_source_tree(
        ...
        bad_root = source_root / "fake_nexify_home"
