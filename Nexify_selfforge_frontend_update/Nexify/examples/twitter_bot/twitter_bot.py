#!/usr/bin/env python3
"""nexify Twitter Bot — @nexifyAI reactive mention handler.
...
DEMO_TWEETS = [
    {
        ...
        "author": "alice_dev",
        "text": "@nexifyAI how do I add a new channel integration?",
    },
    {
        "id": "1000000000000000002",
        "author": "bob_user",
        "text": (
            "@nexifyAI bug: the memory_search tool crashes "
    ...
    },
    {
        ...
        "author": "carol_eng",
        "text": "@nexifyAI it would be great to have a built-in scheduler UI",
    },
    {
        "id": "1000000000000000004",
        "author": "dave_fan",
        "text": "@nexifyAI just discovered this project, absolutely love it!",
    },
    {
        "id": "1000000000000000005",
        "author": "spambot99",
        "text": "@nexifyAI BUY CRYPTO NOW 🚀🚀🚀 LINK IN BIO",
    },
]
...
# nomic-embed-text on the nexify fixture corpus, relevant queries
...
def _build_question_grounded_prompt(
    ...
    return (
        "You are @nexifyAI. Someone asked a question. We retrieved "
...
def _build_question_deferral_prompt(author: str, tweet_id: str, text: str) -> str:
    ...
    """
    return (
        "You are @nexifyAI. Someone asked a question, but our docs "
...
def _build_bug_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @nexifyAI. Someone reported a bug.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
        "1. call http_request to create a github issue:\n"
        "   url: https://api.github.com/repos/open-jarvis/nexify/issues\n"
        "   method: POST\n"
...
def _build_feature_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @nexifyAI. Someone requested a feature.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
        "1. call http_request to create a github issue:\n"
        "   url: https://api.github.com/repos/open-jarvis/nexify/issues\n"
...
def _build_praise_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @nexifyAI. Someone said something nice.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
...
_INJECTION_PROMPT = (
    "Classify this tweet mentioning @nexifyAI as SAFE or MALICIOUS. "
...
def _run_demo(model: str, engine_key: str) -> None:
    ...
    click.echo("nexify Twitter Bot — Demo Mode (reactive only)")
...
_SINCE_ID_STATE_PATH = Path.home() / ".nexify" / "twitter_since_id.txt"
...
def _seed_since_id_to_newest(channel) -> Optional[str]:
    ...
    1. **Persisted state from a prior run** (``~/.nexify/twitter_since_id.txt``).
...
def _run_live(
    ...
    *,
    ...
    try:
        ...
        from nexify.channels.twitter_channel import TwitterChannel
    ...
    click.echo(f"nexify Twitter Bot — {mode_label} Mode")
    click.echo(f"Model: {model}  |  Engine: {engine_key}")
    click.echo("=" * 60)
    ...
    http_restore = None
    if dry_run:
        from nexify.tools.http_request import HttpRequestTool
        ...
        def _dry_http_execute(self, **params):  # noqa: ANN001
            ...
            return ToolResult(
                ...
                content=(
                    '{"number": 999, "html_url": '
                    '"https://github.com/open-jarvis/nexify/issues/999"}'
                ),
            )
    ...
    click.echo(f"\n{mode_hint}")
    click.echo("Waiting for @nexifyAI mentions (poll every 60s). Ctrl+C to stop.\n")
...
def main(
    ...
    """nexify Twitter bot — reactive @nexifyAI mention handler.
