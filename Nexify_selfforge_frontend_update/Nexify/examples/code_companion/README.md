...
A set of developer-focused scripts that use nexify tool-using agents
...
1. **Install nexify** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/nexify/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/nexify/tools/calculator.py \
    --framework unittest \
...
inspect `src/nexify/tools/` for the full registry.
