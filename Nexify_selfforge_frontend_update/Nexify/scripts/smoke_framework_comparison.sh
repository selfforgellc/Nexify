...
SMOKE_FRAMEWORKS=(hermes openclaw nexify)
SMOKE_MODEL="qwen-9b"

for fwk in "${SMOKE_FRAMEWORKS[@]}"; do
  for bench in "${SMOKE_BENCHES[@]}"; do
    uv run python -m nexify.evals.comparison.make_configs \
      --framework "$fwk" \
      --model "$SMOKE_MODEL" \
      --benchmark "$bench" \
...
PY
    ...
    uv run python -m nexify.evals run --config "$config" \
      || echo "    FAILED (continuing)"
  done
done

echo "==> Generating T1 from smoke results"
uv run python -m nexify.evals.comparison.table_gen \
