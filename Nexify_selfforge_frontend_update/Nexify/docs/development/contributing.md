...
nexify uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=nexify --cov-report=html
```
...
nexify uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/nexify/`:
...
try:
    import nexify.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
