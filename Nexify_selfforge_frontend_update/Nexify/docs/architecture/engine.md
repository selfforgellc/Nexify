...
The Engine primitive provides the **inference runtime** -- the layer that connects nexify to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
nexify automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from nexify.engine import discover_engines
...
```python
from nexify.engine import discover_engines, discover_models
...
Engine hosts and defaults are configured in `~/.nexify/config.toml` using **nested per-engine sub-sections**:
...
```python
from nexify.engine._base import messages_to_dicts
...
```python
from nexify.engine import EngineConnectionError
