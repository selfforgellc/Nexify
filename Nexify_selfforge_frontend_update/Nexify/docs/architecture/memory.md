...
- **Persistence:** Data persists across restarts in `~/.nexify/memory.db`
...
from nexify.tools.storage.faiss_backend import FAISSMemory
from nexify.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from nexify.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `nexify.tools.storage.*`.
...
```python
from nexify.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from nexify.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from nexify.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.nexify/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
