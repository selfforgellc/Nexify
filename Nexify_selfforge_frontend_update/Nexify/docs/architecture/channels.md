...
Channel implementations use Python's `threading` module rather than asyncio. This is a deliberate choice: nexify's core inference path is synchronous, and daemon threads are simpler to compose with synchronous code than coroutines.
...
1. Create a new file in `src/nexify/channels/`.
...
```python
from nexify.channels._stubs import BaseChannel, ChannelMessage, ChannelStatus
...
- [API Reference: Channels](../api-reference/nexify/channels/index.md) -- complete class and type signatures
