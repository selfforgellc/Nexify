...
!!! note "Backward compatibility"
    The old `from nexify.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.
...
Wraps the `@anthropic-ai/claude-code` SDK via a bundled Node.js subprocess bridge. Unlike every other agent, inference is handled entirely by the Claude Agent SDK -- the nexify inference engine is not used. This makes `ClaudeCodeAgent` a true external agent, similar in spirit to `OpenHandsAgent` but implemented via subprocess rather than an importable Python SDK.
...
1. On first call, copies the bundled `claude_code_runner/` to `~/.nexify/claude_code_runner/` and runs `npm install --production` if `node_modules` is absent
...
4. Reads stdout and extracts the JSON payload between `---nexify_OUTPUT_START---` and `---nexify_OUTPUT_END---` sentinels
...
- `cleanup_orphans()` removes any stale containers labelled `nexify-sandbox=true`
