...
nexify follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in nexify is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
nexify is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
nexify **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
```bash
jarvis init --force
# Detects hardware, writes ~/.nexify/config.toml with:
# [engine]
...
nexify provides a **clean Python API** through the `Jarvis` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`nexify[server]`, `nexify[memory-colbert]`, etc.)
...
The API server (`jarvis serve`) implements the **OpenAI chat completions API format**, making nexify a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to nexify:
...
nexify requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of nexify.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `nexify[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `nexify[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `nexify[memory-bm25]` | `rank-bm25` |
| API server | `nexify[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `nexify[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `nexify[inference-vllm]` | `vllm` |
| PDF ingestion | `nexify[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `nexify[channel-whatsapp-baileys]` | Node.js 22+ |
