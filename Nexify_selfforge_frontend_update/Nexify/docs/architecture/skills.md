...
2. **Tolerant pass** — maps non-spec vendor fields to canonical locations via `FIELD_MAPPING` table. Unmapped fields are logged and preserved in `metadata.nexify.original_frontmatter`
...
Sidecar storage for optimization output at `~/.nexify/learning/skills/<name>/optimized.toml`:
