...
**Goal:** Implement Spec A — the v1 vLLM-Pearl mining integration — as a new `nexify.mining` subsystem that lets H100/H200 owners running vLLM solo-mine the Pearl PoUW chain with their normal LLM inference.

**Architecture:** New top-level subsystem `mining/` mirrors OJ's existing primitive pattern (peer to `engine/`, `agents/`). A `MiningProvider` ABC + `MinerRegistry` enables future hardware/engine paths (Apple Silicon, AMD, Ollama) without rewrite. The v1 `vllm-pearl` provider orchestrates Pearl's published Docker container; a runtime sidecar at `~/.nexify/runtime/mining.json` decouples mining lifecycle from the existing vLLM engine class. Three deliberate seams (`submit_target` tagged-union, zero-valued `fee_bps`/`fees_owed`, reserved `mining/pools/`) leave room for v2 pool support without retrofit pain.
...
- Absolute imports only (`from nexify.core.registry import ...`).
...
| Path | Responsibility |
|---|---|
| `src/nexify/mining/__init__.py` | Package init; soft-imports providers via `try/except ImportError` |
| `src/nexify/mining/_stubs.py` | `MiningProvider` ABC, `MiningCapabilities`, `MiningConfig`, `MiningStats`, `SoloTarget`/`PoolTarget` tagged union, `Sidecar` dataclass + read/write helpers |
| `src/nexify/mining/_discovery.py` | `detect_providers()`, hardware/Docker/disk/pearld checks, wallet-format check |
| `src/nexify/mining/_constants.py` | `PEARL_REPO`, `PEARL_PINNED_REF`, `PEARL_IMAGE_TAG`, default ports, default model, sidecar path constants |
| `src/nexify/mining/_docker.py` | `PearlDockerLauncher` — `ensure_image()`, `start()`, `stop()`, `is_running()`, `get_logs()` |
| `src/nexify/mining/_collector.py` | `MiningTelemetryCollector` — full impl, shipped unwired in v1; v1.x lights it up in the gateway daemon |
| `src/nexify/mining/vllm_pearl.py` | `VllmPearlProvider` (`MiningProvider` impl); `_parse_gateway_metrics()`; `ensure_registered()` |
| `src/nexify/mining/pools/__init__.py` | Reserved location for v2 `PoolClient` work — empty except for a docstring saying so |
| `src/nexify/cli/mine_cmd.py` | `jarvis mine` Click group: `init`, `start`, `stop`, `status`, `doctor`, `attach`, `logs` |
...
| Path | Change |
|---|---|
| `src/nexify/core/registry.py` | Add `MinerRegistry` class + entry in `__all__` |
| `src/nexify/core/config.py` | Add `MiningConfig` field to `JarvisConfig`; add TOML parser for `[mining]` with `submit_target` tagged-union resolution |
| `src/nexify/engine/_discovery.py` | Add sidecar-aware engine resolution: when `~/.nexify/runtime/mining.json` exists, register a derived `vllm` engine pointing at `vllm_endpoint` |
| `src/nexify/telemetry/store.py` | Add nullable `mining_session_id` column to inference rows; bump `PRAGMA user_version`; helper to tag rows when sidecar is present |
| `src/nexify/cli/__init__.py` | Register the `mine` Click group |
| `src/nexify/cli/hints.py` | Add hint: "mining configured but not running — start it with `jarvis mine start`" |
...
**Files:**
- Modify: `src/nexify/core/registry.py`
...
class MinerRegistry(RegistryBase[Any]):
    ...
    ``nexify.mining._stubs``. Registry keys are short lowercase strings
...
```bash
git add src/nexify/core/registry.py tests/conftest.py tests/core/test_registry.py
...
**Files:**
- Create: `src/nexify/mining/__init__.py`
- Create: `src/nexify/mining/_constants.py`
- Create: `src/nexify/mining/_stubs.py`
- Create: `src/nexify/mining/pools/__init__.py`
...
def test_mining_capabilities_default_unsupported():
    from nexify.mining._stubs import MiningCapabilities
...
def test_solo_target_dataclass():
    from nexify.mining._stubs import SoloTarget
...
def test_pool_target_dataclass():
    from nexify.mining._stubs import PoolTarget
...
def test_mining_config_v1_defaults():
    from nexify.mining._stubs import MiningConfig, SoloTarget
    cfg = MiningConfig(
        provider="vllm-pearl",
...
def test_mining_provider_is_abstract():
    from nexify.mining._stubs import MiningProvider
...
def test_sidecar_write_then_read_roundtrip(sidecar_path: Path, sample_sidecar_payload: dict):
    from nexify.mining._stubs import Sidecar
...
```
Expected: ALL fail with `ImportError` on `nexify.mining._stubs`.

- [ ] **Step 5: Create `_constants.py`**

```python
# src/nexify/mining/_constants.py
...
PEARL_IMAGE_TAG = f"nexify/pearl-miner:{PEARL_PINNED_REF}"
...
RUNTIME_DIR = Path.home() / ".nexify" / "runtime"
...
PEARL_CACHE_DIR = Path.home() / ".nexify" / "cache" / "pearl"
```

- [ ] **Step 6: Create `_stubs.py`**

```python
# src/nexify/mining/_stubs.py
...
from nexify.core.config import HardwareInfo
...
class Sidecar:
    """Read/write helpers for ``~/.nexify/runtime/mining.json``."""
...
from nexify.mining._stubs import (
    MiningCapabilities,
    MiningConfig,
    MiningProvider,
    MiningStats,
...
try:
    from nexify.mining import vllm_pearl  # noqa: F401
...
```python
# src/nexify/mining/pools/__init__.py
...
```bash
git add src/nexify/mining/ tests/mining/__init__.py tests/mining/conftest.py tests/mining/test_stubs.py
...
**Files:**
- Modify: `src/nexify/core/config.py`
...
provider       = "vllm-pearl"
wallet_address = "prl1qexampleaddress"
submit_target  = "pool:https://pool.nexify.ai/submit"
...
def test_mining_config_absent_means_none(tmp_path):
    from nexify.core.config import load_config
...
def test_mining_config_pool_parsed_as_pool_target(tmp_path):
    ...
    assert cfg.mining.submit_target.url == "https://pool.nexify.ai/submit"
```
...
In `src/nexify/core/config.py`, add an import near the top:

```python
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from nexify.mining._stubs import MiningConfig
```
...
In `src/nexify/core/config.py`, locate the existing `load_config()` function (around line 1541). Find the section that processes the loaded TOML data into the `JarvisConfig` (look for where it iterates the `data` dict and assigns to dataclass fields). Add a dedicated handler for `mining` because of the tagged-union parsing — it can't go through the generic walker.
...
def _parse_mining_section(data: dict) -> Optional["MiningConfig"]:
    ...
    from nexify.mining._stubs import MiningConfig, PoolTarget, SoloTarget
...
```bash
git add src/nexify/core/config.py tests/core/test_config.py tests/mining/fixtures/
...
**Files:**
- Create: `src/nexify/mining/_discovery.py`
...
def test_detect_supported_on_h100(hopper_hw):
    from nexify.mining._discovery import detect_for_engine_model
...
def test_check_docker_available_true():
    from nexify.mining._discovery import check_docker_available
    with patch("nexify.mining._discovery._docker_client") as fake:
...
def test_check_disk_free_passes(tmp_path):
    from nexify.mining._discovery import check_disk_free
    with patch("nexify.mining._discovery.shutil.disk_usage") as du:
        # 500 GB free
...
def test_check_pearld_reachable_true():
    from nexify.mining._discovery import check_pearld_reachable
    with patch("nexify.mining._discovery.httpx.post") as post:
...
def test_check_wallet_address_format_valid():
    from nexify.mining._discovery import check_wallet_address_format
...
```python
# src/nexify/mining/_discovery.py
...
def check_disk_free(path: Path) -> Tuple[bool, str]:
    from nexify.mining._constants import MIN_FREE_DISK_GB
...
```bash
git add src/nexify/mining/_discovery.py tests/mining/test_discovery.py
...
**Files:**
- Create: `src/nexify/mining/_docker.py`
...
def test_ensure_image_already_local():
    from nexify.mining._docker import PearlDockerLauncher
    fake = MagicMock()
    fake.images.get.return_value = MagicMock(id="sha256:abc", tags=["nexify/pearl-miner:main"])
    launcher = PearlDockerLauncher(client=fake)
    out = launcher.ensure_image("nexify/pearl-miner:main")
    assert out == "nexify/pearl-miner:main"
    fake.images.get.assert_called_once_with("nexify/pearl-miner:main")
...
def test_ensure_image_falls_back_to_build_for_default_tag():
    from nexify.mining._docker import PearlDockerLauncher
    from nexify.mining._constants import PEARL_IMAGE_TAG
    import docker.errors as derr
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
def test_ensure_image_errors_when_non_default_tag_missing():
    from nexify.mining._docker import PearlDockerLauncher, ImageAcquisitionError
    import docker.errors as derr
    import pytest
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
from nexify.mining._constants import (
    PEARL_CACHE_DIR,
    PEARL_IMAGE_TAG,
    PEARL_PINNED_REF,
    PEARL_REPO,
...
```bash
git add src/nexify/mining/_docker.py tests/mining/test_docker.py
...
**Files:**
- Modify: `src/nexify/mining/_docker.py`
...
def test_launcher_start_calls_run_with_expected_kwargs(_env_password):
    ...
    cfg = MiningConfig(
        ...
        submit_target=SoloTarget(pearld_rpc_url="http://localhost:44107"),
        extra={
            "docker_image_tag": "nexify/pearl-miner:main",
            "model": "pearl-ai/Llama-3.3-70B-Instruct-pearl",
            "vllm_port": 8000,
...
def test_launcher_start_errors_when_password_env_missing():
    from nexify.mining._docker import PearlDockerLauncher, ConfigurationError
    from nexify.mining._stubs import MiningConfig, SoloTarget
    fake = MagicMock()
    launcher = PearlDockerLauncher(client=fake)
    cfg = MiningConfig(
    ...
    with pytest.raises(ConfigurationError) as ei:
        launcher.start(cfg, image="nexify/pearl-miner:main")
...
Append to `src/nexify/mining/_docker.py`:
...
class ConfigurationError(RuntimeError):
    ...
    def start(self, config: "MiningConfig", image: str) -> Any:
        ...
        """
        from nexify.mining._stubs import MiningConfig  # noqa: F401  (typing only)
...
**Files:**
- Modify: `src/nexify/mining/vllm_pearl.py` (creating in next task — for now create the parser as a helper)
- Create: `src/nexify/mining/_metrics.py`
...
def test_parse_gateway_metrics_full():
    from nexify.mining._metrics import parse_gateway_metrics
...
```bash
git add src/nexify/mining/_metrics.py tests/mining/test_metrics.py tests/mining/fixtures/
...
**Files:**
- Create: `src/nexify/mining/vllm_pearl.py`
...
def test_vllm_pearl_detect_supported_on_h100(hopper_hw):
    from nexify.mining.vllm_pearl import VllmPearlProvider
...
async def test_vllm_pearl_start_writes_sidecar(tmp_path, monkeypatch):
    from nexify.mining.vllm_pearl import VllmPearlProvider
    from nexify.mining._stubs import MiningConfig, SoloTarget, Sidecar

    sidecar_path = tmp_path / "mining.json"
    monkeypatch.setattr(
        "nexify.mining.vllm_pearl.SIDECAR_PATH", sidecar_path
    )
...
async def test_vllm_pearl_start_pool_target_raises_not_implemented(monkeypatch, tmp_path):
    from nexify.mining.vllm_pearl import VllmPearlProvider
    from nexify.mining._stubs import MiningConfig, PoolTarget
    ...
    cfg = MiningConfig(
        provider="vllm-pearl",
        wallet_address="prl1qaaa",
        submit_target=PoolTarget(url="https://pool.nexify.ai/submit"),
        extra={"docker_image_tag": "nexify/pearl-miner:main"},
    )
...
async def test_vllm_pearl_stop_removes_sidecar(tmp_path, monkeypatch, written_sidecar):
    from nexify.mining.vllm_pearl import VllmPearlProvider
    monkeypatch.setattr(
        "nexify.mining.vllm_pearl.SIDECAR_PATH", written_sidecar
    )
    fake_client = MagicMock()
    provider = VllmPearlProvider(docker_client=fake_client)
...
def test_vllm_pearl_stats_reads_gateway(monkeypatch, written_sidecar):
    ...
    with patch("nexify.mining.vllm_pearl.httpx.get") as get:
...
def test_ensure_registered_is_idempotent():
    from nexify.core.registry import MinerRegistry
    from nexify.mining.vllm_pearl import (
        VllmPearlProvider,
        ensure_registered,
    )
    ensure_registered()
...
```python
# src/nexify/mining/vllm_pearl.py
    ...
    async def start(self, config: MiningConfig) -> None:
        if isinstance(config.submit_target, PoolTarget):
            raise NotImplementedError(
                "pool support is v2 — see nexify#XYZ. v1 only accepts "
                "submit_target='solo'."
            )
...
```bash
git add src/nexify/mining/vllm_pearl.py tests/mining/test_vllm_pearl.py
...
**Files:**
- Modify: `src/nexify/engine/_discovery.py`
...
Run:
```bash
sed -n '1,80p' src/nexify/engine/_discovery.py
```
...
def test_engine_discovery_picks_up_mining_sidecar(tmp_path, monkeypatch, written_sidecar):
    ...
    """
    from nexify.engine._discovery import discover_engines
    from nexify.mining import _constants as mining_const
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    """
    try:
        from nexify.mining import Sidecar
        from nexify.mining._constants import SIDECAR_PATH
    except ImportError:
        return
    payload = Sidecar.read(SIDECAR_PATH)
    if payload is None:
    ...
    from nexify.engine.openai_compat_engines import OpenAICompatEngine  # adjust to actual class name
...
```bash
git add src/nexify/engine/_discovery.py tests/conftest.py tests/mining/conftest.py tests/engine/test_discovery.py
...
**Files:**
- Create: `src/nexify/mining/_collector.py`
...
async def test_collector_collect_once_returns_stats(written_sidecar):
    from nexify.mining._collector import MiningTelemetryCollector

    sample = (
        "pearl_gateway_shares_submitted_total 50\n"
        "pearl_gateway_shares_accepted_total 49\n"
    )
    with patch("nexify.mining._collector.httpx.get") as get:
        get.return_value.status_code = 200
        get.return_value.text = sample
        store = MagicMock()
...
from nexify.mining._metrics import parse_gateway_metrics
from nexify.mining._stubs import MiningStats, Sidecar
...
```bash
git add src/nexify/mining/_collector.py tests/mining/test_collector.py
...
**Files:**
- Modify: `src/nexify/telemetry/store.py`
...
Run:
```bash
grep -n "PRAGMA user_version\|CREATE TABLE\|ALTER TABLE\|migrate" src/nexify/telemetry/store.py | head -20
```
...
In `src/nexify/telemetry/store.py`:
...
```bash
git add src/nexify/telemetry/store.py tests/telemetry/test_store.py
...
**Files:**
- Create: `src/nexify/cli/mine_cmd.py`
...
def test_mine_doctor_prints_capability_matrix(monkeypatch):
    ...
    with patch("nexify.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("nexify.cli.mine_cmd.check_docker_available", return_value=(True, "running 24.0.7")), \
         patch("nexify.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("nexify.cli.mine_cmd.check_pearld_reachable", return_value=(True, "block height 442107 (synced)")):
...
def test_mine_doctor_flags_unsupported_hardware():
    ...
    with patch("nexify.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("nexify.cli.mine_cmd.check_docker_available", return_value=(True, "ok")), \
         patch("nexify.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("nexify.cli.mine_cmd.check_pearld_reachable", return_value=(False, "connection refused")):
        result = runner.invoke(mine, ["doctor"])
...
from nexify.core.config import HardwareInfo, load_config
from nexify.mining._constants import (
    DEFAULT_PEARL_MODEL,
...
from nexify.mining._discovery import (
    check_disk_free,
...
def mine() -> None:
    ...
    See https://open-jarvis.github.io/nexify/user-guide/mining/ for the
    full guide.
    """
...
```bash
git add src/nexify/cli/mine_cmd.py tests/mining/test_cli.py
...
**Files:**
- Modify: `src/nexify/cli/mine_cmd.py`
...
def test_mine_start_runs_provider_start(monkeypatch):
    ...
    with patch("nexify.cli.mine_cmd.MinerRegistry") as reg, \
         patch("nexify.cli.mine_cmd.load_config") as load, \
         patch("nexify.cli.mine_cmd.asyncio.run") as arun:
        from nexify.mining._stubs import MiningConfig, SoloTarget
...
def test_mine_start_errors_when_no_mining_config():
    from nexify.cli.mine_cmd import mine
    runner = CliRunner()
    with patch("nexify.cli.mine_cmd.load_config") as load:
...
def init(
    ...
    config_path = Path.home() / ".nexify" / "config.toml"
...
def test_mine_status_renders_stats(written_sidecar, monkeypatch):
    from nexify.cli.mine_cmd import mine
    runner = CliRunner()
    monkeypatch.setattr("nexify.cli.mine_cmd.SIDECAR_PATH", written_sidecar)
    sample = (
        "pearl_gateway_shares_submitted_total 100\n"
        "pearl_gateway_shares_accepted_total 99\n"
    ...
    with patch("nexify.mining.vllm_pearl.httpx.get") as get, \
         patch("nexify.cli.mine_cmd.MinerRegistry") as reg:
        get.return_value.status_code = 200
        get.return_value.text = sample
        from nexify.mining.vllm_pearl import VllmPearlProvider
...
def test_mine_attach_writes_sidecar(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd.SIDECAR_PATH", sidecar)
...
def test_mine_logs_streams_container_output(monkeypatch):
    ...
    with patch("nexify.cli.mine_cmd.PearlDockerLauncher",
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        container = docker.from_env().containers.get("nexify-pearl-miner")
...
**Files:**
- Modify: `src/nexify/cli/__init__.py`
- Modify: `src/nexify/cli/hints.py`
...
```bash
sed -n '1,50p' src/nexify/cli/__init__.py
grep -n "add_command\|@main.command\|main.add_command" src/nexify/cli/__init__.py | head -20
```
...
def test_mining_not_running_hint_when_configured_no_sidecar():
    from nexify.cli.hints import mining_not_running_hint
...
```bash
git add src/nexify/cli/__init__.py src/nexify/cli/hints.py tests/cli/test_main.py tests/cli/test_hints.py
...
nexify can mine the [Pearl](https://github.com/pearl-research-labs/pearl)
...
The `nexify.mining` subsystem uses a registry pattern identical to
...
1. Create `src/nexify/mining/<provider>.py`.
2. Subclass `nexify.mining.MiningProvider`.
...
5. Add a soft-import in `mining/__init__.py`:
   ```python
   try:
       from nexify.mining import <provider>  # noqa: F401
...
```markdown
- New mining providers must register via `MinerRegistry` in `src/nexify/core/registry.py` and expose an idempotent `ensure_registered()` per the autouse-clear test convention.
```
...
```bash
uv run pytest tests/mining/ --cov=nexify.mining --cov-report=term-missing
```
...
## Summary
- New `nexify.mining` subsystem with `MiningProvider` ABC + `MinerRegistry`
- `vllm-pearl` provider wrapping Pearl's Docker container
- Runtime sidecar (`~/.nexify/runtime/mining.json`) for engine ↔ mining handoff
