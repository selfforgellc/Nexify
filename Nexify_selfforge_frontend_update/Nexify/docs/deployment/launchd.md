...
nexify includes a launchd property list (plist) for running the API server as a background service on macOS. This provides automatic startup at login, automatic restart if the process exits, and log capture.

## Prerequisites

Before installing the service, ensure that nexify is installed and the `jarvis` command is available at `/usr/local/bin/jarvis`. If you installed via `uv` or `pip` with a different prefix, adjust the path in the plist accordingly.

```bash
git clone https://github.com/open-jarvis/nexify.git && cd nexify && uv sync --extra server
...
```bash
cp deploy/launchd/com.nexify.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.nexify.plist
```
...
```bash
launchctl list | grep nexify
```

You should see a line with the PID and the label `com.nexify`. A `0` in the status column indicates the service is running normally.
...
The provided plist file at `deploy/launchd/com.nexify.plist`:
...
<dict>
    ...
    <string>/tmp/nexify.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/nexify.stderr.log</string>
</dict>
</plist>
```
...
|----------------------|--------------------------------|------------------------------------------------------------------------------------------------------|
| `Label`              | `com.nexify`               | Unique identifier for the service. Used with `launchctl` commands to manage the service.             |
...
| `StandardOutPath`    | `/tmp/nexify.stdout.log`   | File where standard output is written. Contains server startup messages and access logs.             |
| `StandardErrorPath`  | `/tmp/nexify.stderr.log`   | File where standard error is written. Contains error messages and stack traces.                      |
...
cat /tmp/nexify.stdout.log

# View standard error (errors, warnings)
cat /tmp/nexify.stderr.log

# Follow logs in real time
tail -f /tmp/nexify.stdout.log /tmp/nexify.stderr.log
```
...
!!! tip "Persistent log location"
    ...
    ```xml
    <key>StandardOutPath</key>
    <string>/Users/yourname/.nexify/nexify.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/yourname/.nexify/nexify.stderr.log</string>
    ```
...
launchctl unload ~/Library/LaunchAgents/com.nexify.plist
```
...
```bash
# Stop the service
launchctl stop com.nexify

# Start the service
launchctl start com.nexify
```
...
```bash
# List all loaded services matching "nexify"
launchctl list | grep nexify
```
...
| Label  | The service label (`com.nexify`)                           |
...
<dict>
    <key>nexify_ENGINE_DEFAULT</key>
    <string>ollama</string>
    <key>nexify_OLLAMA_HOST</key>
...
The instructions above install the service as a **user agent** (runs only when you are logged in). To run nexify as a system-wide daemon that starts at boot regardless of user login:
...
```bash
sudo cp deploy/launchd/com.nexify.plist /Library/LaunchDaemons/
sudo chown root:wheel /Library/LaunchDaemons/com.nexify.plist
sudo launchctl load /Library/LaunchDaemons/com.nexify.plist
```
