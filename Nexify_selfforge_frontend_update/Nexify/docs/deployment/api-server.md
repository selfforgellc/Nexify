...
nexify includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
The server reads defaults from `~/.nexify/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
The `[server]` section of `~/.nexify/config.toml` controls default server behavior:
...
For production deployments, run nexify behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
