...
The channels module lets nexify send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from nexify.channels.telegram import TelegramChannel
...
from nexify.channels.slack import SlackChannel
...
Channel settings live in the `[channel]` section of `~/.nexify/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.nexify/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from nexify!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  nexify
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. nexify sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since nexify runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from nexify.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from nexify!")
```
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.nexify/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.nexify/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.nexify/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
...
auth_dir = "/home/user/.nexify/whatsapp_baileys_bridge/auth"
...
- [API Reference: Channels](../api-reference/nexify/channels/index.md) -- full class and type signatures
