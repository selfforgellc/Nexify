...
nexify has two types of integrations:
...
```json
{
    "display_information": { "name": "nexify" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "nexify", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @nexify in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "nexify" |
...
Slack serves two purposes in nexify:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [nexify icon](https://github.com/open-jarvis/nexify/blob/main/assets/nexify-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **nexify** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "nexify"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @nexify` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @nexify` in the channel |
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the nexify desktop app
...
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.nexify/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.nexify/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
