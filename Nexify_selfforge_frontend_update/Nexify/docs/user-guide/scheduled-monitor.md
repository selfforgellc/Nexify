...
This writes a pre-configured `~/.nexify/config.toml` for the operative agent with scheduling support.
...
- **All data stays local**: State is stored in `~/.nexify/` using the configured memory backend. Nothing leaves your machine.
