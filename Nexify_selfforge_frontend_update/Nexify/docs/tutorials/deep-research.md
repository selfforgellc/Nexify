...
!!! tip "Prerequisites"
    ...
    - nexify installed: run `uv sync --extra dev` from the repository root
...
1. Creates a `Jarvis` instance targeting the local Ollama engine with `qwen3:8b`. Both parameters are optional — omitting them uses auto-detected defaults from `~/.nexify/config.toml`.
...
- [Getting Started: Configuration](../getting-started/configuration.md) — how to configure engines and models in `~/.nexify/config.toml`
