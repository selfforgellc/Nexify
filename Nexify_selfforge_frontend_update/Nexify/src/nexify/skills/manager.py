...
from nexify.skills.dependency import validate_dependencies
from nexify.skills.executor import SkillExecutor, SkillResult
from nexify.skills.loader import discover_skills
from nexify.skills.tool_adapter import SkillTool
from nexify.skills.types import SkillManifest
from nexify.tools._stubs import BaseTool, ToolExecutor
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.nexify/learning/skills/ if config can't be loaded.
            try:
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.nexify/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.nexify.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from nexify.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("nexify", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["nexify"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.nexify.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("nexify", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.nexify/skills/discovered/``).  Returns a list of dicts with
        ...
        if output_dir is None:
            output_dir = Path("~/.nexify/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "nexify (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.nexify/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from nexify.skills.loader import load_skill_directory
