...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.nexify.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "nexify_meta" the value is
# stored under manifest.metadata["nexify"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("nexify_meta", "platforms"),
    "prerequisites": ("nexify_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.nexify.original_frontmatter,
        and merges metadata.nexify.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize nexify namespace
        oj_meta = dict(raw_metadata.get("nexify") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.nexify.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.nexify.* into canonical fields (these override
        # top-level mappings since they are explicit nexify-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.nexify.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the nexify metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["nexify"] = oj_meta
