...
from nexify.evals.core.dataset import DatasetProvider
...
_RECORDS = [
    {
        "documents": "nexify Architecture Guide (v2.9):\nnexify is organized around five composable primitives: Intelligence, Engine, Agents, Tools, and Learning. The Intelligence primitive handles model definition and catalog management via ModelRegistry. The Engine primitive provides inference through backends including Ollama, vLLM, SGLang, llama.cpp, MLX, and LM Studio. All engines implement the InferenceEngine ABC with generate(), stream(), list_models(), and health() methods.",
        "question": "What are the five primitives of nexify and what does the Engine primitive provide?",
    ...
    },
    {
        ...
        "question": "How does nexify integrate with external MCP tools?",
    ...
    {
        "documents": "Speech Subsystem Technical Reference:\nnexify supports speech-to-text with pluggable backends. SpeechBackend ABC defines transcribe(), health(), and supported_formats() methods. Three backends are available: FasterWhisperBackend (local, CTranslate2, key 'faster-whisper'), OpenAIWhisperBackend (cloud, whisper-1, key 'openai'), and DeepgramSpeechBackend (cloud, nova-2, key 'deepgram'). Auto-discovery prioritizes local backends.",
    ...
    {
        "documents": "Channel System Architecture:\nnexify supports multi-platform messaging via the BaseChannel ABC. Available channels include WhatsAppBaileysChannel, LINEChannel, ViberChannel, MessengerChannel, RedditChannel, MastodonChannel, XMPPChannel, RocketChatChannel, ZulipChannel, TwitchChannel, and NostrChannel. All channels use @ChannelRegistry.register() for discovery and integrate with the EventBus.",
        "question": "How many messaging channels does nexify support and how are they discovered?",
        "answer": "nexify supports 11 messaging channels. They are discovered via the @ChannelRegistry.register() decorator.",
    },
    {
    ...
    {
        "documents": "Desktop Application Guide:\nThe nexify desktop app uses Tauri 2.0. It includes 5 dashboard panels: EnergyDashboard (real-time power monitoring with recharts), TraceDebugger (timeline inspection with step-type color coding), LearningCurve (policy visualization for GRPO/bandit stats), MemoryBrowser (search and stats), and AdminPanel (health, agents, server control). Tauri commands proxy to the nexify REST API.",
    ...
    {
        "documents": "Vault System Guide:\nThe vault provides encrypted credential storage at ~/.nexify/vault.enc using Fernet encryption. The encryption key is auto-generated with 0o600 file permissions for security. CLI commands: 'jarvis vault set KEY' to store, 'jarvis vault get KEY' to retrieve, and 'jarvis vault list' to list stored keys.",
        "question": "What encryption does the vault use and where is it stored?",
        "answer": "The vault uses Fernet encryption and is stored at ~/.nexify/vault.enc.",
    },
    {
    ...
    {
        "documents": "A2A Protocol Implementation:\nnexify implements the Google Agent-to-Agent spec using JSON-RPC 2.0. A2AServer supports tasks/send, tasks/get, and tasks/cancel operations. Agent discovery is available at /.well-known/agent.json. A2AClient enables calling external A2A agents. A2AAgentTool adapts A2A agents as tool-callable resources.",
    ...
    {
        "documents": "Hardware Detection System:\nnexify auto-detects GPU vendor, model, and VRAM using platform-specific tools: nvidia-smi for NVIDIA GPUs, rocm-smi for AMD GPUs, system_profiler for Apple Silicon, and /proc/cpuinfo for CPU features. Based on detection, it recommends the appropriate inference engine (vLLM for NVIDIA, MLX for Apple, llama.cpp for CPU-only).",
        "question": "How does nexify detect hardware and what engine is recommended for Apple Silicon?",
        "answer": "nexify detects hardware using nvidia-smi, rocm-smi, system_profiler, and /proc/cpuinfo. MLX is recommended for Apple Silicon.",
    },
    {
    ...
    {
        "documents": "MCP Quick-Add Feature:\nThe 'jarvis add' command provides quick MCP server setup with 8 built-in templates: github, filesystem, slack, postgres, brave-search, memory, puppeteer, and google-maps. Configuration is saved as JSON to ~/.nexify/mcp/. Each template includes the server command, arguments, and required environment variables.",
        "question": "How many MCP server templates are available via 'jarvis add' and where is configuration saved?",
        "answer": "8 MCP server templates are available. Configuration is saved to ~/.nexify/mcp/.",
    },
    {
