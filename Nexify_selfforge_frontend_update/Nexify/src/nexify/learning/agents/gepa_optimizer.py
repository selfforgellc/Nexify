...
Uses GEPA's adapter pattern to bridge nexify traces into GEPA's
...
from nexify.core.config import GEPAOptimizerConfig
from nexify.core.registry import LearningRegistry
from nexify.learning._stubs import AgentLearningPolicy
...
class nexifyGEPAAdapter:
    """Implements GEPA's adapter protocol for nexify agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'nexify[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = nexifyGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "nexifyGEPAAdapter"]
