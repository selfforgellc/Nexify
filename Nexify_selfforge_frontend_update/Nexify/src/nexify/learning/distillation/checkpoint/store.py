...
A thin wrapper over a local git repository at ``<nexify_home>/.git``.
...
from nexify.learning.distillation.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.nexify/`` (or ``$nexify_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the nexify source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"nexify source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "distillation@nexify.local")
            self._git("config", "user.name", "nexify Distillation")
        ...
        # Allow empty so init succeeds even on a brand-new nexify home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
