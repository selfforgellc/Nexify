...
``~/.nexify/config.toml``, agent prompts, or tool descriptions.
...
def build_diagnostic_tools(
    *,
    ...
    config :
        Dict with ``config_path`` (Path) and ``nexify_home`` (Path).
    benchmark_samples :
    ...
    """
    nexify_home = config["nexify_home"]
    ...
    def _get_agent_prompt(agent_name: str) -> str:
        prompt_path = nexify_home / "agents" / agent_name / "system_prompt.md"
        try:
    ...
    def _get_tool_description(tool_name: str) -> str:
        desc_path = nexify_home / "tools" / "descriptions.toml"
        try:
    ...
    return [
        ...
        DiagnosticTool(
            ...
            description="Read the current nexify config.toml.",
