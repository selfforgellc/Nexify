...
def channels_status() -> None:
    ...
    from nexify.channels.imessage_daemon import is_running
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "nexify.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from nexify.connectors.retriever import (
            TwoStageRetriever,
        )
        from nexify.connectors.store import KnowledgeStore
        from nexify.engine.ollama import OllamaEngine
        from nexify.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from nexify.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from nexify.channels.imessage_daemon import stop_daemon
