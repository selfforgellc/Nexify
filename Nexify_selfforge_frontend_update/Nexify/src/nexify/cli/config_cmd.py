...
def _get_config_path(path: str | None) -> Path:
    ...
    if path:
        return Path(path)
    return Path(os.environ.get("nexify_CONFIG", DEFAULT_CONFIG_PATH))


def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    """Display detected hardware information."""
    from nexify.core.config import detect_hardware, recommend_engine
...
def set_config(key: str, value: str) -> None:
    ...
    from nexify.core.config import DEFAULT_CONFIG_DIR, validate_config_key
    ...
    config_path = Path(
        os.environ.get("nexify_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
