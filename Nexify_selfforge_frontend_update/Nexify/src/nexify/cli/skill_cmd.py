...
def _get_discovered_dir() -> Path:
    ...
    return Path("~/.nexify/skills/discovered/").expanduser()


def _get_overlay_dir() -> Path:
    """Return the directory where optimization overlays are stored."""
    return Path("~/.nexify/learning/skills/").expanduser()
...
def _get_skill_paths() -> List[Path]:
    ...
    user_dir = Path("~/.nexify/skills/").expanduser()
...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from nexify.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from nexify.skills.sources.openclaw import OpenClawResolver
    ...
    if source == "github":
        ...
        cache = _Path(
            "~/.nexify/skill-cache/github/" + url.rstrip("/").rsplit("/", 1)[-1]
        ).expanduser()
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.nexify/skills/`` and ``./skills`` for a directory whose
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.nexify/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="nexify-discover-dryrun-"))
        try:
