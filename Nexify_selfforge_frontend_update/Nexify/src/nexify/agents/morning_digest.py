...
def _load_persona(persona_name: str) -> str:
    ...
    search_paths = [
        Path("configs/nexify/prompts/personas") / f"{persona_name}.md",
        Path.home() / ".nexify" / "prompts" / "personas" / f"{persona_name}.md",
    ]
...
class MorningDigestAgent(ToolUsingAgent):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        evaluator_feedback = ""
        try:
            from nexify.agents.digest_evaluator import DigestEvaluator
