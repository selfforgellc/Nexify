"""Load system prompt and few-shot overrides from $nexify_HOME.
...
- System prompts: ``$nexify_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$nexify_HOME/agents/{name}/few_shot.json``
...
def _nexify_home() -> Path:
    """Resolve $nexify_HOME, defaulting to ~/.nexify."""
    return Path(os.environ.get("nexify_HOME", "~/.nexify")).expanduser()


def load_system_prompt_override(agent_name: str) -> str | None:
    """Return the override prompt for *agent_name*, or ``None``.

    Looks for ``$nexify_HOME/agents/<agent_name>/system_prompt.md``.
    ``nexify_HOME`` defaults to ``~/.nexify`` when unset.
    """
    home = _nexify_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$nexify_HOME/agents/<agent_name>/few_shot.json``.
