"""API key authentication middleware for the nexify server."""
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, api_key: str = "") -> None:  # noqa: ANN001
        super().__init__(app)
        self._api_key = api_key or os.environ.get("nexify_API_KEY", "")
...
def check_bind_safety(host: str, *, api_key: str) -> None:
    ...
    *api_key* is empty.
    ...
    if not is_loop and not api_key:
        logger.error(
            "Binding to %s requires nexify_API_KEY to be set. "
