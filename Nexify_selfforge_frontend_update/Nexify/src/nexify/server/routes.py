...
from nexify.server.models import (
    ChatCompletionChunk,
        ...
        try:
            ...
            if query_text:
                ...
                if len(enriched) > len(messages):
                    from nexify.server.models import ChatMessage
            ...
            logging.getLogger("nexify.server").debug(
...
def _handle_agent(
    ...
    if audio_path:
        ...
        from nexify.server.models import AudioMeta
...
async def _handle_agent_stream(agent, bus, model, req):
    ...
    from nexify.server.stream_bridge import create_agent_stream
...
async def _handle_stream(
    ...
    from nexify.server.cloud_router import (
        is_cloud_model,
        stream_cloud,
        stream_local,
    )
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("nexify.server").error(
                "Stream error: %s",
                exc,
                exc_info=True,
            )
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from nexify.server.cloud_router import is_cloud_model, list_local_models
...
async def reload_cloud_engine(request: Request):
    ...
    # Re-read ~/.nexify/cloud-keys.env and update the running process env.
    keys_path = Path.home() / ".nexify" / "cloud-keys.env"
    if keys_path.exists():
...
async def security_scan():
    ...
    from nexify.cli.scan_cmd import PrivacyScanner
