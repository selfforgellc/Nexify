"""Single point of contact between Python and the Rust ``nexify_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``nexify_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import nexify_rust  # type: ignore[import-untyped]
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from nexify.security.injection_scanner import (
        InjectionScanResult,
    )
    from nexify.security.types import ScanFinding, ThreatLevel
