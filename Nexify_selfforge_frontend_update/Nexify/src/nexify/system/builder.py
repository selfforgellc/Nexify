...
from nexify.core.events import EventBus, get_event_bus
...
class SystemBuilder:
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.nexify/agents.db").expanduser()
                )
    ...
    def _resolve_memory(self, config):
        try:
            import nexify.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            ...
            from nexify.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from nexify.sandbox.runner import ContainerRunner
    ...
    def _setup_learning_orchestrator(config: JarvisConfig):
        ...
        try:
            from nexify.core.config import DEFAULT_CONFIG_DIR
            from nexify.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from nexify.learning.training.lora import LoRATrainingConfig
            from nexify.traces.store import TraceStore
