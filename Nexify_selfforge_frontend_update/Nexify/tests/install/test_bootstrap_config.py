"""Tests for nexify.cli._bootstrap.write_initial_config."""
...
def test_writes_minimal_local_config(tmp_nexify_home: Path) -> None:
    hw = HardwareInfo(
        platform="linux",
        cpu_brand="AMD EPYC",
        cpu_count=16,
    ...
    cfg_path = tmp_nexify_home / "config.toml"
...
def test_writes_cloud_config(tmp_nexify_home: Path) -> None:
    ...
    data = tomllib.loads((tmp_nexify_home / "config.toml").read_text())
    ...
    assert "sk-ant-test" not in (tmp_nexify_home / "config.toml").read_text()


def test_includes_install_provenance(tmp_nexify_home: Path, monkeypatch) -> None:
...
def test_writes_seed_files_if_absent(tmp_nexify_home: Path) -> None:
    hw = HardwareInfo(platform="linux", cpu_brand="x", cpu_count=1, ram_gb=4.0)
    _bootstrap.write_initial_config(hardware=hw, engine="ollama", model="qwen3.5:2b")
    assert (tmp_nexify_home / "SOUL.md").exists()
    assert (tmp_nexify_home / "MEMORY.md").exists()
    assert (tmp_nexify_home / "USER.md").exists()
    assert (tmp_nexify_home / "skills").is_dir()


def test_does_not_overwrite_existing_seeds(tmp_nexify_home: Path) -> None:
    soul = tmp_nexify_home / "SOUL.md"
...
def test_overwrites_existing_config_toml(tmp_nexify_home: Path) -> None:
    cfg = tmp_nexify_home / "config.toml"
...
def test_handles_special_chars_in_model_name(tmp_nexify_home: Path) -> None:
