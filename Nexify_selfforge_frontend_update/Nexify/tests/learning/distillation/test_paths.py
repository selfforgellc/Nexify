"""Tests for nexify.learning.distillation.storage.paths module."""
...
class TestResolveDistillationRoot:
    ...
    def test_default_is_under_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from nexify.learning.distillation.storage import paths

        monkeypatch.delenv("nexify_HOME", raising=False)
        result = paths.resolve_distillation_root()
        assert result == Path.home() / ".nexify" / "learning"

    def test_respects_nexify_home_env_var(
        self,
        tmp_path: Path,
        ...
        monkeypatch.setenv("nexify_HOME", str(custom))
        result = paths.resolve_distillation_root()
    ...
    def test_returns_absolute_path(
        ...
        monkeypatch.setenv("nexify_HOME", str(tmp_path / "rel"))
        result = paths.resolve_distillation_root()
    ...
    def test_rejects_path_inside_source_tree(
        ...
        # Find the nexify source root by walking up from the paths module.
        source_root = paths._find_source_root()
        assert source_root is not None  # We must be running inside the repo.

        # Force nexify_HOME to point inside the source tree.
        monkeypatch.setenv("nexify_HOME", str(source_root / "junk_dir"))
...
class TestEnsureDistillationDirs:
    ...
    def test_creates_subdirs(
        ...
        monkeypatch.setenv("nexify_HOME", str(tmp_path / "oj"))
