...
- TraceStore at ~/.nexify/traces.db with some traces
"""
...
def real_trace_store():
    ...
    db_path = Path.home() / ".nexify" / "traces.db"
    if not db_path.exists():
        pytest.skip("No traces.db found at ~/.nexify/")
...
class TestDiagnosisRunnerLive:
    ...
    def test_diagnosis_produces_output(
        ...
        from nexify.learning.distillation.diagnose.runner import (
            DiagnosisRunner,
        )
...
class TestColdStartLive:
    ...
    def test_orchestrator_cold_start_with_no_feedback(
        ...
        from nexify.learning.distillation.gate.cold_start import (
