"""Tests for nexify.evals.comparison.table_gen."""
...
from nexify.evals.comparison.table_gen import (  # noqa: E402
    MixedCommitError,
    ResultsFrame,
    load_results,
    main,
...
class TestRenderBooktabs:
    def test_emits_valid_tabular(self) -> None:
        from nexify.evals.comparison.table_gen import _render_booktabs
        ...
        assert "hermes" in fragment and "nexify" in fragment
...
class TestT1Builder:
    def test_t1_builds_from_synthetic_results(self) -> None:
        from nexify.evals.comparison.table_gen import (
            ResultsFrame,
            _build_t1,
        )
        ...
        assert "nexify-distilled" in fragment.lower() or "OJ-distilled" in fragment
...
class TestT3to7Builders:
    ...
    def test_builder_emits_tabular(self, builder_name: str) -> None:
        import nexify.evals.comparison.table_gen as m

        builder = getattr(m, builder_name)
        rows = []
        for fwk in ["hermes", "nexify"]:
...
class TestTableBuilderRegistry:
    def test_registry_has_all_seven(self) -> None:
        from nexify.evals.comparison.table_gen import _TABLE_BUILDERS
...
class TestTableGenCLI:
    def test_cli_writes_fragment_and_preview(self, tmp_path: Path) -> None:
        ...
        from nexify.evals.comparison.table_gen import main
