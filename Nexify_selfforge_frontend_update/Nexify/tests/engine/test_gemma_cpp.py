...
from nexify.core.config import EngineConfig, GemmaCppEngineConfig
...
class TestMessagesToPrompt:
    def _make_engine(self):
        ...
        from nexify.engine.gemma_cpp import GemmaCppEngine
...
class TestGemmaCppLifecycle:
    ...
    @patch("nexify.engine.gemma_cpp._import_pygemma")
    ...
    def test_generate_warns_on_model_mismatch(self, mock_import) -> None:
        ...
        from nexify.engine import gemma_cpp as gc_mod
...
class TestGemmaCppHealth:
    def test_health_true_when_configured_and_files_exist(self, tmp_path) -> None:
        ...
        with patch("nexify.engine.gemma_cpp._import_pygemma"):
    ...
    def test_health_false_when_pygemma_missing(self, tmp_path) -> None:
        ...
        with patch(
            "nexify.engine.gemma_cpp._import_pygemma",
...
class TestGemmaCppDiscovery:
    def test_host_map_contains_gemma_cpp(self) -> None:
        from nexify.engine._discovery import _HOST_MAP

        assert "gemma_cpp" in _HOST_MAP
        assert _HOST_MAP["gemma_cpp"] is None

    def test_make_engine_passes_config(self) -> None:
        from nexify.core.config import GemmaCppEngineConfig, JarvisConfig
        from nexify.core.registry import EngineRegistry
        from nexify.engine._discovery import _make_engine
        from nexify.engine.gemma_cpp import GemmaCppEngine
