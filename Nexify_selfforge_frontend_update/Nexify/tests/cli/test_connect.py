...
def test_connect_list_no_connectors() -> None:
    ...
    runner = CliRunner()
    with mock.patch(
        "nexify.cli.connect_cmd.connect.__wrapped__"
        if hasattr(cli, "__wrapped__")
        else "nexify.core.registry.ConnectorRegistry.items",
        return_value=(),
    ):
        with mock.patch(
            "nexify.core.registry.ConnectorRegistry.items",
            return_value=(),
        ):
...
def test_connect_specific_source(tmp_path: object) -> None:
    ...
    with (
        mock.patch(
            "nexify.core.registry.ConnectorRegistry.contains",
            return_value=True,
        ),
        mock.patch(
            "nexify.core.registry.ConnectorRegistry.get",
