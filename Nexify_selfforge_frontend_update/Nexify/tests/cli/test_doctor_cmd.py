...
from nexify.cli.doctor_cmd import (
...
class TestDoctorRuns:
    def test_doctor_runs(self) -> None:
        ...
        with (
            patch("nexify.cli.doctor_cmd.load_config", return_value=mock_config),
            patch(
                "nexify.cli.doctor_cmd.DEFAULT_CONFIG_PATH",
                Path("/tmp/nonexistent/config.toml"),
            ),
            patch("nexify.cli.doctor_cmd._check_engines", return_value=[]),
            patch("nexify.cli.doctor_cmd._check_models", return_value=[]),
        ):
...
class TestCheckEngineProbing:
    def test_check_engine_probing(self) -> None:
        ...
        from nexify.cli.doctor_cmd import CheckResult
...
class TestCheckDefaultModel:
    def test_empty_default_model_is_not_a_warning(self) -> None:
        ...
        mock_config = MagicMock()
        mock_config.intelligence.default_model = ""
        with patch("nexify.cli.doctor_cmd.load_config", return_value=mock_config):
