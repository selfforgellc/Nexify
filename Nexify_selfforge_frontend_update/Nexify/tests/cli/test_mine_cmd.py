...
def test_pearl_base_model_lookup_prefers_validated_scalingintelligence_artifacts():
    from nexify.mining._models import pearl_variant_for_base_model
...
def test_mine_inspect_model_validated_artifact_passes(monkeypatch) -> None:
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd._hf_json", fake_hf_json)
...
def test_mine_init_writes_mining_config(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(
        ...
        env={"nexify_CONFIG": str(config_path)},
    )
...
def test_mine_init_writes_cuda_visible_devices_for_vllm(
    ...
    with (
        patch("nexify.cli.mine_cmd._detect_hardware"),
        patch(
            "nexify.cli.mine_cmd.detect_for_engine_model",
            return_value=MiningCapabilities(supported=True),
        ),
        patch(
            "nexify.cli.mine_cmd.check_docker_available",
            return_value=(True, ""),
        ),
        patch("nexify.cli.mine_cmd.check_disk_free", return_value=(True, "")),
        patch("nexify.cli.mine_cmd._docker_from_env", return_value=MagicMock()),
        patch(
            "nexify.cli.mine_cmd.PearlDockerLauncher.ensure_image",
            return_value="nexify/pearl-miner:master",
        ),
    ):
        result = CliRunner().invoke(
            cli,
...
"""
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    ...
    with patch("nexify.cli.mine_cmd._provider_ids", return_value=("cpu-pearl",)):
        with patch("nexify.cli.mine_cmd.MinerRegistry.contains", return_value=True):
            with patch(
                "nexify.cli.mine_cmd.MinerRegistry.get",
...
def test_mine_status_reports_sidecar_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd.SIDECAR_PATH", sidecar_path)
    monkeypatch.setattr("nexify.cli.mine_cmd._pid_alive", lambda pid: True)
    ...
    monkeypatch.setattr(
        "nexify.cli.mine_cmd._stats_from_metrics_url",
...
def test_mine_stop_terminates_pids_and_removes_sidecar(
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd._terminate_pid", fake_terminate)
...
def test_mine_doctor_without_config(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd.SIDECAR_PATH", tmp_path / "none.json")

    with patch("nexify.cli.mine_cmd._provider_ids", return_value=()):
        result = CliRunner().invoke(
            cli,
            ["mine", "doctor"],
            env={"nexify_CONFIG": str(config_path)},
...
def test_mine_validate_model_allows_planned_with_runtime_evidence(
    ...
    monkeypatch.setattr(
        "nexify.cli.mine_cmd._post_json",
...
def test_mine_validate_model_falls_back_to_vllm_metrics(
    ...
    monkeypatch.setattr("nexify.cli.mine_cmd._stats_from_metrics_url", fake_stats)
