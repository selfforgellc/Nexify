...
8. **`jarvis-uninstall`** removes `~/.nexify/` and `~/.local/bin/jarvis*`. Verify with `ls`.
...
- Delete `~/.nexify/config.toml` — verify bare `jarvis` re-runs init.
- Delete `~/.nexify/.venv` — verify re-running curl heals it.
