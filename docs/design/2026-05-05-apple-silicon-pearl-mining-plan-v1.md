...
src/nexify/mining/__init__.py  # Soft-import cpu_pearl
src/nexify/mining/_constants.py  # Add PEARL_PINNED_REF and helper constants
```
...
**Files:**
- Modify: `src/nexify/mining/_constants.py:1-N` (created in Spec A)
...
Run: `cat src/nexify/mining/_constants.py`
...
Append to `src/nexify/mining/_constants.py`:
...
CPU_PEARL_LOCAL_CLONE_DIR = "~/.nexify/cache/pearl"
...
Run: `uv run ruff check src/nexify/mining/_constants.py`
Expected: no errors.

- [ ] **Step 4: Commit**

```bash
git add src/nexify/mining/_constants.py
...
**Files:**
- Create: `src/nexify/mining/_install.py`
...
```python
"""Tests for nexify.mining._install."""
...
def test_pearl_packages_available_returns_false_when_pearl_mining_missing():
    from nexify.mining import _install
...
def test_install_hint_is_actionable():
    ...
    from nexify.mining._install import install_hint
...
Expected: FAIL — `ImportError: cannot import name '_install' from 'nexify.mining'`
...
```bash
git add src/nexify/mining/_install.py tests/mining/test_install.py
...
**Files:**
- Modify: `src/nexify/mining/_install.py`
...
Run: `uv run pytest tests/mining/test_install.py -v`
Expected: FAIL — `AttributeError: module 'nexify.mining._install' has no attribute 'build_from_pin'`

- [ ] **Step 3: Implement `build_from_pin`**

Append to `src/nexify/mining/_install.py`:
...
Run: `uv run ruff check src/nexify/mining/_install.py tests/mining/test_install.py`
Expected: no errors.
...
**Files:**
- Create: `src/nexify/mining/_miner_loop_main.py`
...
```python
"""Tests for nexify.mining._miner_loop_main."""
from __future__ import annotations
...
def test_decode_mining_info_returns_header_and_target(fake_mining_info_response):
    from nexify.mining._miner_loop_main import _decode_mining_info
...
def test_encode_plain_proof_round_trips():
    ...
    from nexify.mining._miner_loop_main import _encode_plain_proof
...
def test_jsonrpc_envelope_shape():
    ...
    from nexify.mining._miner_loop_main import _make_request
...
Run with:
    python -m nexify.mining._miner_loop_main \
...
logger = logging.getLogger("nexify.mining.miner_loop")
...
Run: `uv run ruff check src/nexify/mining/_miner_loop_main.py tests/mining/test_miner_loop.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/nexify/mining/_miner_loop_main.py tests/mining/test_miner_loop.py
...
**Files:**
- Create: `src/nexify/mining/_pearl_subprocess.py`
...
```python
"""Tests for nexify.mining._pearl_subprocess."""
from __future__ import annotations
...
def test_launcher_start_spawns_two_processes(fake_popen, tmp_path):
    from nexify.mining._pearl_subprocess import PearlSubprocessLauncher
...
- ``nexify.mining._miner_loop_main`` (this repo), which polls the gateway
    ...
    def start(self, *, m: int, n: int, k: int, rank: int) -> None:
        ...
        miner_loop = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "nexify.mining._miner_loop_main",
                "--gateway-host",
                self.gateway_host,
                "--gateway-port",
...
Run: `uv run ruff check src/nexify/mining/_pearl_subprocess.py tests/mining/test_pearl_subprocess.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/nexify/mining/_pearl_subprocess.py tests/mining/test_pearl_subprocess.py
...
**Files:**
- Modify: `src/nexify/mining/_pearl_subprocess.py`
...
Run: `find ~/.nexify/cache/pearl/miner/pearl-gateway -name config.py -path '*pearl_gateway*' 2>/dev/null || find / -name config.py -path '*pearl_gateway*' 2>/dev/null | head -3`

Expected: `pearl-gateway/src/pearl_gateway/config.py` (path depends on where Pearl was cloned). If the file isn't on disk yet, run `python -m nexify.mining._install` (added in Task 4) or clone Pearl manually first.
...
Open `src/nexify/mining/_pearl_subprocess.py` and replace each guessed env name with the actual one. Keep the dict structure; only the keys change.
...
```bash
git add src/nexify/mining/_pearl_subprocess.py
...
**Files:**
- Create: `src/nexify/mining/cpu_pearl.py`
...
```python
"""Tests for nexify.mining.cpu_pearl.CpuPearlProvider."""
from __future__ import annotations
...
def darwin_apple_hw():
    ...
    from nexify.mining._stubs import HardwareInfo, GpuInfo
...
def windows_hw():
    ...
    from nexify.mining._stubs import HardwareInfo

    return HardwareInfo(platform="win32", cpu_arch="x86_64", gpu=None)


def test_detect_supported_on_apple_silicon(darwin_apple_hw):
    from nexify.mining.cpu_pearl import CpuPearlProvider

    with patch(
        "nexify.mining._install.pearl_packages_available", return_value=True
    ):
...
def test_detect_unsupported_when_pearl_not_installed(darwin_apple_hw):
    ...
    with patch(
        "nexify.mining._install.pearl_packages_available", return_value=False
    ):
...
Run: `uv run ruff check src/nexify/mining/cpu_pearl.py tests/mining/test_cpu_pearl.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/nexify/mining/cpu_pearl.py tests/mining/test_cpu_pearl.py
...
**Files:**
- Modify: `src/nexify/mining/cpu_pearl.py`
...
def test_start_writes_sidecar_and_returns_running(darwin_apple_hw, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nexify.mining.cpu_pearl._sidecar_path",
    ...
    with patch(
        "nexify.mining.cpu_pearl.PearlSubprocessLauncher",
...
Replace `src/nexify/mining/cpu_pearl.py` with the full implementation:
...
def _sidecar_path() -> Path:
    return Path(os.path.expanduser("~/.nexify/runtime/mining.json"))


def _log_dir() -> Path:
    return Path(os.path.expanduser("~/.nexify/logs/mining"))
...
Run: `uv run ruff check src/nexify/mining/cpu_pearl.py`
Expected: no errors.
...
**Files:**
- Modify: `src/nexify/mining/__init__.py`
...
Run: `cat src/nexify/mining/__init__.py`
...
Append to `src/nexify/mining/__init__.py` (or modify if Spec A already defined `ensure_registered`):

```python
def _register_cpu_pearl() -> None:
    """Register CpuPearlProvider in MinerRegistry. Idempotent."""
    from nexify.core.registry import MinerRegistry  # type: ignore
...
# Call at import time so the registry is populated as soon as nexify.mining
...
```bash
git add src/nexify/mining/__init__.py
...
nexify can mine the [Pearl](https://github.com/pearl-research-labs/pearl) chain
...
| `mine status` shows `last_error: gateway metrics unreachable` | `pearl-gateway` crashed | Check `~/.nexify/logs/mining/pearl-gateway.log` |
...
For anything not on this list, capture `~/.nexify/logs/mining/` and open
an issue at https://github.com/open-jarvis/nexify/issues.
...
def test_provider_runs_end_to_end_on_this_host(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nexify.mining.cpu_pearl._log_dir",
...
- `MiningProvider` ABC at `src/nexify/mining/_stubs.py`
- `MinerRegistry` class at `src/nexify/core/registry.py`
- `MiningConfig`, `MiningCapabilities`, `MiningStats`, `HardwareInfo` dataclasses
- Sidecar location convention `~/.nexify/runtime/mining.json`
