---
title: Configuration
description: Complete reference for nexify configuration
---

# Configuration

nexify uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
nexify creates the `~/.nexify/` directory and populates it with a default config when you run `jarvis init`.
...
3. Writes `~/.nexify/config.toml` with sensible defaults
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, nexify automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, nexify uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
| `db_path` | string | `~/.nexify/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
[traces]
enabled = false
db_path = "~/.nexify/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.nexify/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.nexify/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.nexify/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `jarvis init`, nexify probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.nexify/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure nexify entirely from Python without a TOML file:
...
nexify respects the following environment variables:
