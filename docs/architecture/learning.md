...
!!! note "Backward compatibility"
    The canonical locations are now `nexify.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `nexify.core.types` (for `RoutingContext`). The old `nexify.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `nexify.learning._stubs`.
...
```python
from nexify.learning.router import HeuristicRouter, build_routing_context
...
```python
from nexify.learning.trace_policy import TraceDrivenPolicy
...
```python
from nexify.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from nexify.learning.sft_policy import SFTPolicy
```
...
```python
from nexify.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from nexify.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from nexify.learning.heuristic_reward import HeuristicRewardFunction
...
over nexify's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `nexify-learning`
