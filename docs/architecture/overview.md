...
description: The five-primitive architecture behind nexify — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
nexify is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![nexify Architecture](../assets/nexify_Architecture.png)
...
The sandbox module (`nexify.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in nexify use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
