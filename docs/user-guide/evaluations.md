...
The nexify evaluation framework (`nexify-evals`) measures model **correctness and accuracy** on academic datasets. It is a separate package from the main nexify library and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    nexify has two distinct measurement systems that complement each other:

    | System | Package | Measures | Entry Point |
    |--------|---------|----------|-------------|
    | **Evaluations** | `nexify-evals` | Correctness on academic datasets (accuracy, pass rate) | `nexify-eval` |
    | **Benchmarks** | `nexify` | Engine performance (latency, throughput) | `jarvis bench` |
...
The evaluation framework is a standalone package in the `evals/` directory. Install it alongside nexify:

```bash
uv sync --extra eval
```

This installs the `nexify-eval` CLI entry point and all required dependencies (`datasets`, `huggingface-hub`, `tqdm`, `rich`).
...
These benchmarks evaluate models on practical tasks that mirror real nexify use cases.
...
```bash
uv run python -m nexify.evals --config src/nexify/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run python -m nexify.evals --config src/nexify/evals/configs/use_case_v2_local.toml
```
...
```bash
nexify-eval run-all -m qwen3:8b

# With options
nexify-eval run-all -m gpt-4o -n 100 --output-dir results/gpt4o/
```
...
```bash
nexify-eval summarize results/supergpqa_qwen3-8b.jsonl
```
...
```bash
nexify-eval run --config src/nexify/evals/configs/full-suite.toml
```
...
nexify-eval run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Python SDK](python-sdk.md) — Programmatic access to nexify inference and agents
