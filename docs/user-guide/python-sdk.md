...
The nexify Python SDK provides a high-level interface for interacting with local inference engines, managing memory, and running agent workflows. The primary entry point is the `Jarvis` class.
...
If no `config` or `config_path` is provided, the SDK loads configuration from the default location (`~/.nexify/config.toml`), falling back to built-in defaults.
...
| `version` | `str`          | The nexify version string     |
