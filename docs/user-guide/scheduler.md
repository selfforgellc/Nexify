...
!!! note "Optional component"
    The scheduler is a standalone module (`nexify.scheduler`). It is not wired into the default `Jarvis` / `JarvisSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `jarvis scheduler start`.
...
# Set up storage
store = SchedulerStore(db_path="~/.nexify/scheduler.db")  # (1)!
...
Scheduler settings live in the `[scheduler]` section of `~/.nexify/config.toml`.

```toml title="~/.nexify/config.toml"
[scheduler]
enabled = false
db_path = "~/.nexify/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.nexify/scheduler.db`     | SQLite database path                       |
