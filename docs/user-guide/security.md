...
nexify includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.
...
from nexify.security.guardrails import GuardrailsEngine
from nexify.security.scanner import SecretScanner
from nexify.security.types import RedactionMode
...
from nexify.security.scanner import PIIScanner
...
from nexify.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.nexify/audit.db", bus=bus)
...
import time
from nexify.security.audit import AuditLogger
from nexify.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.nexify/audit.db")
...
Security settings live in the `[security]` section of `~/.nexify/config.toml`.
...
secret_scanner = true
pii_scanner = true
audit_log_path = "~/.nexify/audit.db"
...
| `audit_log_path` | `str` | `~/.nexify/audit.db` | Path to the SQLite audit log |
...
import re
from nexify.security._stubs import BaseScanner
from nexify.security.types import ScanFinding, ScanResult, ThreatLevel
...
- [API Reference: Security](../api-reference/nexify/security/index.md) — full class and function signatures
