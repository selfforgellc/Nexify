...
Each script follows the same SDK pattern: create a `Jarvis` instance, call `j.ask()` with an agent and tools, print the result, and close the instance. The schedule is managed externally by the nexify scheduler daemon.
...
!!! note "Cron expression syntax"
    nexify uses standard five-field cron syntax: `minute hour day-of-month month day-of-week`. Install `croniter` (`uv add croniter`) for full expression support including ranges and step values. Without it, basic `hour:minute` patterns still work.
...
from nexify.scheduler import TaskScheduler
...
j = Jarvis()  # uses defaults from ~/.nexify/config.toml
response = j.ask(
...
from nexify.channels import ChannelRegistry
