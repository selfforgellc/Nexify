...
logger = logging.getLogger("nexify.server.agent_manager")
...
def _make_lightweight_system(
    ...
    try:
        ...
        try:
            from nexify.core.events import get_event_bus
...
def _ensure_registries_populated() -> None:
    ...
    from nexify.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import nexify.channels  # noqa: F401
    except Exception:
        pass

    try:
        import nexify.tools  # noqa: F401
    except Exception:
        pass

    # Also try to import browser tools (not included in nexify.tools.__init__)
    for _browser_mod in ("nexify.tools.browser", "nexify.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("nexify.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("nexify.tools.")
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "nexify.tools.browser",
            "nexify.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from nexify.core.credentials import TOOL_CREDENTIALS
    from nexify.core.registry import ChannelRegistry, ToolRegistry
...
def _build_deep_research_tools(
    ...
    if not knowledge_db_path:
        from nexify.core.config import DEFAULT_CONFIG_DIR
    ...
    from nexify.tools.knowledge_sql import KnowledgeSQLTool
    from nexify.tools.scan_chunks import ScanChunksTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from nexify.mcp.transport import StdioTransport, StreamableHTTPTransport
    from nexify.tools.mcp_adapter import MCPToolProvider
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from nexify.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from nexify.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from nexify.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from nexify.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from nexify.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from nexify.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from nexify.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from nexify.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from nexify.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from nexify.core.events import EventType, get_event_bus
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.nexify/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from nexify.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from nexify.core.credentials import get_credential_status
