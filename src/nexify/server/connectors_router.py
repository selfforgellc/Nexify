...
def _ensure_connectors_registered() -> None:
    ...
    try:
        import nexify.connectors  # noqa: F401
    except Exception:
        pass
    ...
    if not ConnectorRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("nexify.connectors.")
...
def create_connectors_router():
    ...
    async def connector_detail(connector_id: str):
        ...
        oauth_setup = None
        try:
            from nexify.connectors.oauth import (
    ...
    async def connect_connector(connector_id: str, req: ConnectRequest):
        ...
        if instance.is_connected():
            ...
            def _ingest() -> None:
                try:
                    from nexify.connectors.pipeline import (
                        IngestionPipeline,
                    )
                    from nexify.connectors.store import KnowledgeStore
    ...
    async def oauth_callback(
        ...
        return HTMLResponse(
            content=(
                ...
                "<p>You can close this tab and return to nexify.</p>"
