...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import nexify.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from nexify.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from nexify.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from nexify.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from nexify.tools.agent_tools import AgentSendTool
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from nexify.tools.storage.ingest import ingest_path
...
async def list_skills(request: Request):
    ...
    try:
        from nexify.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.nexify/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from nexify.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP nexify_requests_total Total requests processed",
            "# TYPE nexify_requests_total counter",
            f"nexify_requests_total {stats.get('total_requests', 0)}",
            "# HELP nexify_tokens_total Total tokens generated",
            "# TYPE nexify_tokens_total counter",
            f"nexify_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP nexify_latency_avg_ms Average latency in milliseconds",
            "# TYPE nexify_latency_avg_ms gauge",
            f"nexify_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
def include_all_routes(app) -> None:
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from nexify.server.agent_manager_routes import (  # noqa: PLC0415
