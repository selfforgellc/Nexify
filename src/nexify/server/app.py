"""FastAPI application factory for the nexify API server."""
...
from nexify.server.api_routes import include_all_routes
from nexify.server.comparison import comparison_router
from nexify.server.connectors_router import create_connectors_router
from nexify.server.dashboard import dashboard_router
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(
        title="nexify API",
        description="OpenAI-compatible API server for nexify",
        version="0.1.0",
    )
    ...
    try:
        from nexify.server.middleware import create_security_middleware
    ...
    if webhook_config:
        try:
            from nexify.server.webhook_routes import (
