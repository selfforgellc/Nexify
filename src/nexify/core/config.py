...
User configuration lives at ``~/.nexify/config.toml``.  ``load_config()``
...
DEFAULT_CONFIG_DIR = Path.home() / ".nexify"
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from nexify.intelligence.model_catalog import BUILTIN_MODELS
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.nexify/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through nexify. You are not a cloud service. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.nexify/whatsapp_auth
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "nexify-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.nexify/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.nexify/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.nexify/SOUL.md"
    memory_path: str = "~/.nexify/MEMORY.md"
    user_path: str = "~/.nexify/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.nexify/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/nexify/skill-index.git"
    index_dir: str = "~/.nexify/skill-index/"
    max_depth: int = 5
...
@dataclass
class JarvisConfig:
    """Top-level configuration for nexify."""
...
def validate_config_key(dotted_key: str) -> type:
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import nexify.core.config as _cfg_mod
...
def load_config(path: Optional[Path] = None) -> JarvisConfig:
    ...
    ----------
    path:
        Explicit config file. If not set, uses ``nexify_CONFIG`` when set,
        otherwise ``~/.nexify/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    ...
    elif os.environ.get("nexify_CONFIG"):
        config_path = Path(os.environ["nexify_CONFIG"]).expanduser().resolve()
    else:
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.nexify/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.nexify/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.nexify/whatsapp_auth
