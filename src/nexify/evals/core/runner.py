...
from nexify.evals.core.export import _hardware_info_dict
from nexify.evals.core.scorer import Scorer
from nexify.evals.core.tracker import ResultTracker
from nexify.evals.core.types import (
    EvalRecord,
    EvalResult,
    MetricStats,
    RunConfig,
...
try:
    from nexify.telemetry.efficiency import compute_efficiency
except ImportError:  # pragma: no cover
    compute_efficiency = None  # type: ignore[assignment]

try:
    from nexify.telemetry.efficiency import estimate_model_flops_per_token
except ImportError:  # pragma: no cover
...
class EvalRunner:
    ...
    def run(
        ...
        from nexify.evals.core.dataset import DatasetProvider as _DP
    ...
    def _process_one(self, record: EvalRecord) -> EvalResult:
        ...
        def _backend_error_result(full: dict, message: str) -> EvalResult:
            ...
            return EvalResult(
                ...
                framework=full.get(
                    "framework",
                    getattr(self._backend, "framework_name", "nexify"),
                ),
        ...
        try:
            ...
            return EvalResult(
                ...
                framework=full.get("framework", "nexify"),
            ...
            return EvalResult(
                ...
                model_answer="",
                error=str(exc),
                framework=getattr(self._backend, "framework_name", "nexify"),
