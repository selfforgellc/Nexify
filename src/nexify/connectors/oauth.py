...
class OAuthProvider:
    ...
    # Filenames in ~/.nexify/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.nexify/connectors/`` and falls
    back to environment variables ``nexify_{NAME}_CLIENT_ID`` and
    ``nexify_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"nexify_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to nexify.</p>"
                    b"</body></html>"
                )
