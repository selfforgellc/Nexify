...
from nexify.core.config import HardwareInfo, detect_hardware, load_config
...
from nexify.mining._models import (
    get_pearl_model_spec,
    iter_pearl_model_specs,
    pearl_variant_for_base_model,
)
from nexify.mining._stubs import Sidecar
from nexify.mining.vllm_pearl import ensure_registered as ensure_vllm_registered
...
def _ensure_providers_registered() -> None:
    ...
    from nexify.mining.cpu_pearl import ensure_registered as ensure_cpu_registered

    ensure_cpu_registered()
    try:
        from nexify.mining.apple_mps_pearl import (
...
def init(
    ...
    from nexify.core.config import DEFAULT_CONFIG_PATH

    config_path = Path(os.environ.get("nexify_CONFIG", DEFAULT_CONFIG_PATH))
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        launcher._container = client.containers.get("nexify-pearl-miner")
    except Exception as exc:  # noqa: BLE001
