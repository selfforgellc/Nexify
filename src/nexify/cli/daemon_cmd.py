...
from nexify.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the nexify server daemon."""
...
def start(
    ...
    """Start the nexify server as a background daemon."""
    ...
    cmd = [sys.executable, "-m", "nexify.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]nexify server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running nexify server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the nexify server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the nexify server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
