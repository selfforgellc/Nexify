...
from nexify.cli._bootstrap import detect_cloud_keys
from nexify.cli.model import find_model_spec, hf_download, ollama_pull
...
def init(
    ...
    """Detect hardware and generate ~/.nexify/config.toml."""
    console = Console()
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "nexify" / "examples"
        )
