"""Check for newer nexify releases on GitHub."""
...
_CACHE_PATH = Path("~/.nexify/version-check.json").expanduser()
_CACHE_TTL = 86400  # 24 hours
_GITHUB_API = "https://api.github.com/repos/open-jarvis/nexify/releases/latest"
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            sys.stderr.write(
                f"\033[33mA new version of nexify is available "
                f"(v{current} \u2192 v{latest})\n"
                f"Update: cd ~/nexify && git pull && uv sync\033[0m\n\n"
            )
