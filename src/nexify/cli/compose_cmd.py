...
@click.group()
def compose() -> None:
    """Compose, run, benchmark, and deploy nexify configurations.
...
def compose_list(kind: Optional[str]) -> None:
    ...
    try:
        from nexify.recipes.loader import discover_recipes

        recipes = discover_recipes(kind=kind)
        if not recipes:
            console.print("[dim]No compositions found.[/dim]")
            console.print(
                "[dim]Place TOML recipes in src/nexify/recipes/data/ "
                "or ~/.nexify/recipes/[/dim]"
            )
            return
...
def compose_show(name: str) -> None:
    ...
    console = Console(stderr=True)
    try:
        from nexify.recipes.loader import resolve_recipe
...
def compose_bench(
    ...
    try:
        ...
        try:
            from nexify.evals.cli import _run_single
        except ImportError:
...
def compose_stop(name: str) -> None:
    ...
    try:
        ...
        from nexify.recipes.loader import _PROJECT_OPERATORS_DIR
