...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.nexify is inside iCloud Drive sync scope."""
        try:
            config_path = Path("~/.nexify").expanduser().resolve()
            icloud_path = Path("~/Library/Mobile Documents/").expanduser().resolve()
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.nexify may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.nexify is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]nexify Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with nexify."
        )
        console.print()
    elif warn_count:
        console.print(
