"""``jarvis doctor`` — run diagnostic checks on the nexify installation."""
...
from nexify.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _ensure_engines_imported() -> None:
    ...
    try:
        import nexify.engine  # noqa: F401
    except Exception:
        pass
...
def _check_engines() -> List[CheckResult]:
    ...
    from nexify.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "nexify[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "nexify[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "nexify[energy-amd]", "AMD energy monitoring"),
        ("colbert", "nexify[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "nexify[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your nexify installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]nexify Doctor[/bold]")
    console.print()
    ...
    from nexify.cli._bg_state import get_status
    ...
    if bg.rust_extension == "ready":
        ...
        console.print(
            "    retry: ~/.nexify/.scripts/install-rust.sh && "
            "~/.nexify/.scripts/build-extension.sh"
        )
        bg_failed = True
    else:
        console.print(
    ...
    for model_id, state in bg.models.items():
        if state == "ready":
            ...
            console.print(f"    retry: ~/.nexify/.scripts/pull-model.sh {model_id}")
            bg_failed = True
        else:
