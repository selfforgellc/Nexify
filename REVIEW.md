# nexify PR Review Instructions

You are reviewing pull requests for nexify, a local-first personal AI agent framework built with Python, Rust (PyO3), and TypeScript.
...
- **Registry pattern compliance** — new components (engines, tools, agents, channels) must register via `ToolRegistry`, `EngineRegistry`, `AgentRegistry`, `ChannelRegistry`, etc. in `src/nexify/core/registry.py`
...
- **Event bus integration** — new lifecycle events should use `EventBus` from `src/nexify/core/events.py`
